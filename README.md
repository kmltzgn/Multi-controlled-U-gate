...
This project builds a Qiskit circuit for a multi-controlled single-qubit unitary gate $C^n(U)$. The notebook writes a Qiskit function that takes two inputs: a positive integer $n$ and a matrix $U \in \text{U}(2)$ and outputs a quantum circuit on $n + 1$ qubits, with further ancillas, that implements a multi-controlled $U$ gate, $C^nU$
